...
public partial class Cam_Internal_Mark_Calculation : System.Web.UI.Page
{
    ...
    protected void btngo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            strvaluecheckquery = strvaluecheckquery + " select distinct Istype,s.subject_no,sy.syll_code,isnull(t.sections,'') as from tbl_cam_calculation t,subject s,syllabus_master sy,Degree d,Course c where s.subject_no=t.subject_no and s.syll_code=sy.syll_code and sy.degree_code=d.Degree_Code and d.Course_Id=c.Course_Id " + stredulevelvalue + "";
            ...
            string strsubjectquery = "select distinct (str(r.batch_year)+'-'+c.course_Name+'['+d.acronym+']'+'-'+str(r.current_semester)) as degree,c.Course_Name,r.Current_Semester,r.degree_code,d.Acronym ,subject_name,s.subject_no,s.syll_code,r.batch_year,isnull(r.Sections,'') as Sections from registration r,subject s,subjectchooser sc,syllabus_master sm,Sub_sem Sem,degree d,course c ";
            ...
            if (ckhdegreewise.Checked == true)
            {
                ...
                if (Session["Staff_Code"] == null || Session["Staff_Code"] == "")
                {
                    strsubjectquery = "select distinct S.subject_no,subject_name,s.syll_code,sm.batch_year,sm.degree_code,r.current_semester,de.acronym,c.course_name,ltrim(rtrim(isnull(r.sections,''))) as Sections from subject as S,syllabus_master  as SM,";
...
//--------------------old Calculation
                        ...
                        for (int calcount = 0; dscalculate.Tables[0].Rows.Count > calcount; calcount++)
                        {
                            ...
                            if (sumcriteria.Trim() == "0")
                            {
                                ...
                                { //added by Mullai
                                    ...
                                    // CalMark1 = CalMark1 + CalMark;
                                    // Calmaxmark1 = CalMark1 + Calmaxmark;
                                    //CalMark = 0;
                                    //Calmaxmark = 0;
                                }
                                else
                ...
                if (syllcode != "" && subjectno != "")
                {
                    ...
                    if (dsgetdetails.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int a = 0; a < dsgetdetails.Tables[0].Rows.Count; a++)
                        {
                            ...
                            string subjectId = Convert.ToString(dsgetdetails.Tables[0].Rows[a]["subjectid"]);
                            if (!string.IsNullOrEmpty(subjectId))
    ...
    protected void gridview1_OnDataBound(object sender, EventArgs e)
    {
        ...
        //    //string l3 = row.Cells[0].Text;
        //    //string l4 = (previousRow.Cells[0].Text);
        //    string l3 = GridView1.Rows[rowIndex].Cells[1].Text;
        //    string l4 = GridView1.Rows[rowIndex+1].Cells[1].Text;
        //    if (l3 == l4)
        //    {
