        ...
        try
        {
            ...
            if (dictstdets.Count > 0)
            {
                ...
                else if (chkcommonforall.Checked && chkSameSub.Checked == false)
                {
                    ...
                    //{
                    //    string sections = FpSpread1.Sheets[0].Cells[i, 3].Text.ToString();
                    //    string subno = FpSpread1.Sheets[0].Cells[i, 5].Tag.ToString();
                    //    string maxmark = FpSpread1.Sheets[0].Cells[i, 12].Tag.ToString();
                    //    string minmark = FpSpread1.Sheets[0].Cells[i, 11].Tag.ToString();
                    //    //string syllcode = FpSpread1.Sheets[0].Cells[Convert.ToInt32(activerow), 6].Tag.ToString();
                    //    string tempsyllCode = Convert.ToString(FpSpread1.Sheets[0].Cells[i, 6].Tag);

                    //    if (tempsyllCode.Trim() == syllcode.Trim())
                    //    {
                    ...
                    //        }
                    //        string insupdatevalues = "if not exists(select * from Exam_type where criteria_no='" + critreiano + "' and batch_year='" + ddlbatch.SelectedValue.ToString() + "' and subject_no='" + subno + "' and sections='" + sections + "')";
                    //        insupdatevalues = insupdatevalues + " insert into Exam_type (criteria_no,subject_no,exam_date,batch_year,max_mark,min_mark,sections,new_minmark,new_maxmark,islock,elockdate,durationNew,duration,examFromTime,examToTime)";//examFromTime,examToTime
                    //        insupdatevalues = insupdatevalues + " values('" + critreiano + "','" + subno + "','" + dte.ToString("MM/dd/yyyy") + "','" + ddlbatch.SelectedValue.ToString() + "','" + newmaxmark + "','" + newminmark + "','" + sections + "','" + newminmark + "','" + newmaxmark + "','" + isloc + "','" + lockdate + "','" + newduartion + "','" + newduartion + "','" + dtStartDateTime.ToString("HH:mm") + "','" + dtEndDateTime.ToString("HH:mm") + "')";
                    //        insupdatevalues = insupdatevalues + " else";
                    //        insupdatevalues = insupdatevalues + " update Exam_type set max_mark='" + newmaxmark + "',min_mark='" + newminmark + "',new_maxmark='" + newmaxmark + "',new_minmark='" + newminmark + "',exam_date='" + dte.ToString("MM/dd/yyyy") + "',islock='" + isloc + "', elockdate='" + lockdate + "',durationNew='" + newduartion + "',duration='" + newduartion + "',examFromTime='" + dtStartDateTime.ToString("HH:mm") + "',examToTime='" + dtEndDateTime.ToString("HH:mm") + "' where criteria_no='" + critreiano + "' and batch_year='" + ddlbatch.SelectedValue.ToString() + "' and subject_no='" + subno + "' and sections='" + sections + "'";
                    //        insupdate = da.update_method_wo_parameter(insupdatevalues, "text");
                    //        string examcode = dirAcc.selectScalarString("select exam_code from Exam_type where criteria_no='" + critreiano + "' and batch_year='" + ddlbatch.SelectedValue.ToString() + "' and subject_no='" + subno + "' and sections='" + sections + "'");
                    //        savefunnew(examcode, GridView1);
                    //    }
                    //}
                    #endregion
